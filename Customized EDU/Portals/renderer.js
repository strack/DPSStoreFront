...
if (e == null)
	...
	folios.render = function (_d){
		...
		var filteredCards = (foliofilter&&foliofilter.length ? _.filter(foliodata, function(fd){
			return fd.filter.indexOf(foliofilter)>=0;
		}) : foliodata);
		...
		_.each(filteredCards, function(d,i){
			...
			if (_.indexOf(filters,d.filter) ==-1) filters.push(d.filter);
		});


		var filterButtons = d3.select('#top').selectAll('div.filterbutton').data(filters);
