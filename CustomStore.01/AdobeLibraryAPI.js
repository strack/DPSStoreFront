...
BaseFolio.prototype.download = function () {
    // TODO: Change to isDownloadable
    if (this.state != FolioState.ENTITLED) {
        throw new Error('Attempting to download a baseFolio with state: ' + this.state);
    }
...
Folio.prototype.purchase = function () {
    if (this.state != FolioState.PURCHASABLE) {
        throw new Error('Attempting to purchase a folio with state: ' + this.state);
    }
...
FolioStateChangingTransaction.prototype.start = function () {
    ...
    for (i = 0; i < this.folio.currentTransactions.length; i++) {
        ...
        if (transaction !== this && transaction instanceof FolioStateChangingTransaction) {
...
AuthenticationService.prototype._update = function (data) {
    if (data.hasOwnProperty("update")) {
        ...
        if (update.hasOwnProperty("currentTransaction")) {
            this.currentTransaction = TransactionManager.instance._allTransactions[update.currentTransaction];
        }
    ...
    if (data.hasOwnProperty("update")) {
        ...
        if (update.hasOwnProperty("subscriptionReceipts")) {
            for (subscriptionReceiptId in update.subscriptionReceipts) {
                ...
                Log.instance.debug("\nReceiptService created Subscription Receipt: " + JSON.stringify(receipt));
                this.registerNewReceipt(receipt, true);
                adds.push(receipt);
            }
        }
